...
**Определим среднее число пакетов в системе ($N$):**
...
**Определим среднюю общую задержку ($d$):**
...
**Применим формулу Литтла для нахождения средней скорости поступления пакетов ($a$):**
...
#### Решение
**Временная разница прибытия пакетов ($R_S < R_C$, первая линия - узкое место):**
...
**Условия для второй линии как узкого места ($R_C < R_S$):**
...
$t_{\text{P1\textunderscore освобождает\textunderscore R2}} = (L/R_S + d_{\text{распр1}}) + L/R_C$
...
$t_{\text{P2\textunderscore прибытие\textunderscore марш}} = (L/R_S + T) + L/R_S + d_{\text{распр1}}$
...
$t_{\text{P2\textunderscore прибытие\textunderscore марш}} \geq t_{\text{P1\textunderscore освобождает\textunderscore R2}}$
...
**Найдем $\Delta$ (среднее время, необходимое для отправки объекта по каналу доступа):**
```math
...
**Найдем общее среднее время ответа (без кэша):**
...
**Найдем общее время ответа с кэширующим сервером:**
...
**Ответ:**
$\Delta = \frac{17}{300} \text{ с} \approx 0.0567 \text{ с}$.
Общее среднее время ответа без кэша: $T_{\text{общ}} \approx 3.6071 \text{ с}$.
Общее среднее время ответа с кэшем: $T'_{\text{общ}} \approx 1.2356 \text{ с}$.
