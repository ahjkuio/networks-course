...
*   Общая скорость канала: $R_{\\text{канала}} = 150$ бит/с
*   Кол-во параллельных соединений: $N_{\\text{соедин}} = 10$
*   Скорость на одно соединение: $R = R_{\\text{канала}} / N_{\\text{соедин}} = 150 / 10 = 15$ бит/с
*   Размер объекта данных: $L_{\\text{данных}} = 100\\,000$ бит
...
$$T_{\\text{данных}} = \\frac{L_{\\text{данных}}}{R} = \\frac{100000}{15} \\approx 6666.667 \\text{ с}$$
...
$$T_{\\text{объект\\_непост}} = 3(T_{\\text{упр}} + T_p) + (T_{\\text{данных}} + T_p) = 3T_{\\text{упр}} + T_{\\text{данных}} + 4T_p$$
$$T_{\\text{объект\\_непост}} \\approx 3(13.333) + 6666.667 + 4T_p \\approx 40 + 6666.667 + 4T_p \\approx 6706.667 \\text{ с} + 4T_p$$

**2. Параллельные непостоянные HTTP-соединения**

*   Загрузка базового HTML (1 объект):
    $T_{\\text{HTML}} = T_{\\text{объект\\_непост}} \\approx 6706.667 \\text{ с} + 4T_p$
*   Параллельная загрузка 10 встроенных объектов (каждый по новому соединению, время определяется самым долгим, т.е. временем одного такого объекта):
    $T_{\\text{10\\_встроенных}} = T_{\\text{объект\\_непост}} \\approx 6706.667 \\text{ с} + 4T_p$
*   Общее время (последовательно: сначала HTML, потом параллельно встроенные):
    $$T_{\\text{непост}} = T_{\\text{HTML}} + T_{\\text{10\\_встроенных}} = 2 \\cdot T_{\\text{объект\\_непост}}$$
    $$T_{\\text{непост}} = 2 \\cdot (3T_{\\text{упр}} + T_{\\text{данных}} + 4T_p) = 6T_{\\text{упр}} + 2T_{\\text{данных}} + 8T_p$$
...
*   Установка 10 TCP-соединений параллельно (время на установку одного: $2 \\text{ RTT}_{\\text{упр}}$ без учета данных = $2(T_{\\text{упр}} + T_p)$ ):
    $T_{\\text{уст\\_TCP}} = 2T_{\\text{упр}} + 2T_p \\approx 2(13.333) + 2T_p \\approx 26.666 \\text{ с} + 2T_p$
*   Загрузка базового HTML по одному из соединений (1 запрос + 1 ответ):
    $T_{\\text{загрузка\\_HTML}} = (T_{\\text{упр}} + T_p) + (T_{\\text{данных}} + T_p) = T_{\\text{упр}} + T_{\\text{данных}} + 2T_p \\approx 13.333 + 6666.667 + 2T_p \\approx 6680 \\text{ с} + 2T_p$
*   Параллельная загрузка 10 встроенных объектов по оставшимся 9 и одному уже использованному соединению (время определяется загрузкой одного объекта, т.к. параллельно):
    $T_{\\text{10\\_встроенных\\_пост}} = T_{\\text{упр}} + T_{\\text{данных}} + 2T_p \\approx 6680 \\text{ с} + 2T_p$
*   Общее время (сначала установка всех TCP, затем параллельно HTML и затем параллельно 10 встроенных. Поскольку HTML и 10 встроенных объектов не могут загружаться строго одновременно, если HTML должен быть получен первым, то этапы суммируются):
    $$T_{\\text{пост}} = T_{\\text{уст\\_TCP}} + T_{\\text{загрузка\\_HTML}} + T_{\\text{10\\_встроенных\\_пост}} $$
    $$T_{\\text{пост}} = (2T_{\\text{упр}} + 2T_p) + (T_{\\text{упр}} + T_{\\text{данных}} + 2T_p) + (T_{\\text{упр}} + T_{\\text{данных}} + 2T_p)$$
    $$T_{\\text{пост}} = 4T_{\\text{упр}} + 2T_{\\text{данных}} + 6T_p$$
...
Это эквивалентно экономии $2T_{\\text{упр}} + 2T_p$, т.е. времени установки одного TCP-соединения (если считать 2 RTT).
